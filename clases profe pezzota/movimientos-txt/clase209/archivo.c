...
int crear_lotes_cuentas(t_cuenta *cuentas, int cant_registros)
{
    FILE *pf = fopen("cuentas.dat","wb");
    if(!pf)
    {
        return -1;
    }

    fwrite(cuentas, sizeof(t_cuenta)*cant_registros,1,pf);
...
int crear_lotes (t_movimiento *socios, int cant_registros)
{
    ...
    fwrite(socios, sizeof(t_movimiento)*cant_registros,1,pf);
...
int actualizar_cuentas()
{
    ...
    FILE * pm = fopen("movimientos.txt", "rt");
    ...
    finmov = fscanf(pm, "%d %c %f",&movimientos.nro_cuenta, &movimientos.tipo_mov, &movimientos.importe);
    while (!feof(pc) && finmov != -1)
    {
